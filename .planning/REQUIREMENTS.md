...
### Discovery & Matching (DISC)

- [ ] **DISC-01**: Пользователь может создать и редактировать набор фильтров: ключевые слова, СПГЗ-коды, регион, диапазон суммы — для автоматического отбора подходящих тендеров
- [ ] **DISC-02**: ARQ-воркер периодически (каждые 15 минут) получает пачками новые/изменённые тендеры из goszakup batch API и апсертит их в локальную БД
- [ ] **DISC-03**: Воркер матчинга сравнивает новые тендеры с фильтрами каждого пользователя и создаёт записи `tender_match` (статусы: matched → notified → skipped | participating)
- [ ] **DISC-04**: Пользователь видит «Подборку» — ленту совпавших тендеров с карточками (название, заказчик, сумма, дедлайн, регион, источник, статус матча)
- [ ] **DISC-05**: Пользователь получает Telegram-уведомление о новом совпавшем тендере с кнопками «Участвуем» / «Пропустить»; «Участвуем» создаёт заявку через `application_service.create()` и включает Pipeline Phase 5 (функционал активен после Phase 6)
- [ ] **DISC-06**: В боковом меню присутствует ссылка на Telegram-бот TenderIt
